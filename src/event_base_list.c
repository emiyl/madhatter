...
int mh_event_base_list_load(mh_event_base_list_data *data, const uint8_t *blob, size_t len) {
    ...
    for (size_t i = 0u; i < count; ++i) {
        if (reader.pos + 6u > reader.len) {
            mh_event_base_list_free(data);
            return -1;
        }
        data->entries[i].id_event = (uint16_t)(reader.data[reader.pos] | (reader.data[reader.pos + 1] << 8));
        data->entries[i].index_puzzle = (int16_t)(reader.data[reader.pos + 2] | (reader.data[reader.pos + 3] << 8));
        data->entries[i].index_event_viewed_flag = (int16_t)(reader.data[reader.pos + 4] | (reader.data[reader.pos + 5] << 8));
        reader.pos += 6u;
    }
    return 0;
}
...
int mh_event_base_list_save(const mh_event_base_list_data *data, mh_buffer *out) {
    ...
    for (size_t i = 0u; i < data->count; ++i) {
        ...
        if (mh_writer_write_u16_le(&writer, (uint16_t)(int16_t)data->entries[i].index_puzzle) != 0) {
            mh_writer_free(&writer);
            return -1;
        }
        if (mh_writer_write_u16_le(&writer, (uint16_t)(int16_t)data->entries[i].index_event_viewed_flag) != 0) {
            mh_writer_free(&writer);
            return -1;
        }
    }
